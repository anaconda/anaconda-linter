...
def test_cdts_for_linux_only_valid(file: str) -> None:
    ...
    """
    assert_no_lint_message(recipe_file=file, lint_check="cdts_for_linux_only", arch="linux-aarch64")


@pytest.mark.parametrize(
    "file,msg_count",
    [
        ("cdts_for_linux_only/cdts_for_non_linux.yaml", 5),
    ],
)
def test_cdts_for_linux_only_invalid(file: str, msg_count: int) -> None:
    ...
    msg_title = [f"The CDT package {{{{ cdt('{cdt}') }}}} should only be included for Linux" for cdt in cdts]
    assert_lint_messages(
        recipe_file=file,
